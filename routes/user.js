  ...
  try {
    ...
    if (decodedUserId == userId && resp?.is_valid) {
      ...
      if (getUser?.length < 1) {
        ...
        const trialPlan = await query(`SELECT * FROM plan WHERE is_trial = 1 LIMIT 1`, []);
        if (trialPlan.length > 0) {
          await updateUserPlan(trialPlan[0], uid);
          await query(`UPDATE user SET trial = 1 WHERE uid = ?`, [uid]);
        }
...
router.post("/signup", async (req, res) => {
  try {
    ...
    const getTrialPlan = await query(`SELECT * FROM plan WHERE is_trial = 1 LIMIT 1`, []);
    if (getTrialPlan.length > 0) {
      await updateUserPlan(getTrialPlan[0], uid);
      await query(`UPDATE user SET trial = 1 WHERE uid = ?`, [uid]);
    }
...
router.get("/get_me", validateUser, async (req, res) => {
  try {
    ...
    const user = data[0];
    const now = Date.now();
    const planExpire = user?.plan_expire ? parseInt(user.plan_expire) : null;
    const daysLeft = planExpire ? Math.ceil((planExpire - now) / (1000 * 60 * 60 * 24)) : 0;
    const isTrialActive = user?.trial == 1 && daysLeft > 0;
    const isTrialExpired = user?.trial == 1 && daysLeft <= 0;

    res.json({
      data: { ...user, contact: contact.length, daysLeft, isTrialActive, isTrialExpired },
      success: true,
      addon: finalAddon,
    });
  } catch (err) {
